#include <stdio.h>
int main(int argc, char const *argv[])
{
    double nota1, nota2, nota3, notaf, media;
    scanf("%lf", &nota1);
    scanf("%lf", &nota2);
    scanf("%lf", &nota3);
    media = ((nota1+nota2+nota3)/2);
    if (media>=7.0)
    {printf("Você está aprovado!");}
    else{
       if (media<3){
       printf("Nos veremos novamente semestre que vem 0_0");}
       else{
          if (media>=5&&nota1>=3){if (nota2>=3&&nota3>=3){printf("escapou fedendo!");}}
          else{
              printf("Ficou para reposição");

              if (nota1<=nota2 && nota1<=nota3) {if (nota1<=notaf){
                 media = ((notaf+nota2+nota3)/2);
                 if ((media>=7.0)||((media>=5)&&(notaf>=3)))
                     {printf("Passou na recuperação!");}
                 else
                     {printf("Lhe dou meio ponto, ops!! quer dizer: nos veremos novamente semestre que vem  :~~~~");}
                     }}
              
              if (nota2<=nota1 && nota2<=nota3){if (nota2<=notaf){
                 media = ((notaf+nota2+nota3)/2);
                 if ((media>=7.0)||((media>=5)&&(notaf>=3)))
                     {printf("Passou na recuperação!");}
                 else
                     {printf("Lhe dou meio ponto, ops!! quer dizer: nos veremos novamente semestre que vem  :~~~~");}
                     }}
              
              if (nota1<=nota2 && nota1<=nota3){if (nota1<=notaf){
                 media = ((notaf+nota2+nota3)/2);
                 if ((media>=7.0)||((media>=5)&&(notaf>=3)))
                     {printf("Passou na recuperação!");}
                 else
                     {printf("Lhe dou meio ponto, ops!! quer dizer: nos veremos novamente semestre que vem  :~~~~");}
                     }}
}}}
return 0;
}